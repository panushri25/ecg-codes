...
void computeMSE()
{
	...
	for (I=0; I<NSAMPLES; I=I+2)
	{
		...
		for (SI=0; SI<NSIGMAS; SI++)
		{
			...
			hcdata[SI*NSAMPLES+I] = ((p00+p10) + (p20+p30) + (p40 + p50));
			...
			hcdata[SI*NSAMPLES+secondIndex] = ((p01+p11) + (p21+p31) + (p41 + p51));
...
//		uint32_t start_time = getClockTime();
		...
		FILE * pFile;
		int i;
		pFile = fopen ("hcdata.txt","a");
		if (pFile!=NULL)
		{
			for(i=0 ; i<NSAMPLES ; i++)
			{
			fprintf(pFile,"%f",hcdata[best_sigma_index*NSAMPLES+i]);
			fprintf(pFile,"\n");
			}
		}
	fclose (pFile);
